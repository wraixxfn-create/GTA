...
$('#app').innerHTML=`
  <main class="app-shell">
    ...
    <header class="topbar">
      ...
      <div class="topbar-center"><span class="topbar-index">01 / GEOGRAPHIC FOUNDATION</span><span class="topbar-divider"></span><span>LOWMERE COASTAL REGION</span></div>
      <div class="topbar-actions"><div class="live-indicator"><span class="live-dot"></span> LIVE TERRAIN</div><button class="topbar-atlas" id="open-atlas">${icon.atlas}<span>OPEN ATLAS</span><kbd>M</kbd></button></div>
    </header>

    <aside class="sidebar" aria-label="Regional map information and reserved districts">
      <div class="sidebar-intro">
        <div class="eyebrow"><span class="eyebrow-rule"></span> FIELD NOTES <span class="eyebrow-no">/ VOL. 01</span></div>
        <h1>A coast with<br><em>room to grow.</em></h1>
        <p>One original stretch of land. A sheltered bay, a broken northern ridge, and more than one way across the water.</p>
      </div>
      ...
      <div class="sidebar-section-heading"><span>FUTURE FOOTPRINTS</span><span class="heading-count">12 RESERVED</span></div>
      <div id="district-list" class="district-list" role="list" aria-label="Reserved future districts"></div>
      <div class="sidebar-note"><span class="note-symbol">◌</span><span>Geographic foundation only.<br>No districts or buildings constructed.</span></div>
    </aside>
    ...
    <div class="right-panel">
      ...
      <div class="map-key"><span><i class="key-forest"></i> WOODLAND</span><span><i class="key-road"></i> ROUTES</span><span><i class="key-zone"></i> RESERVED</span></div>
      <section id="selected-card" class="selected-card" aria-live="polite"></section>
    </div>

    <div class="scene-tools"><button id="toggle-boundaries" class="tool-button is-on" title="Toggle reservation boundaries (B)">${icon.layers}<span>BOUNDARIES</span><small>ON</small></button><button id="reset-camera" class="tool-button" title="Return to the central bay">${icon.target}<span>RESET VIEW</span></button></div>
    <footer class="statusbar"><div class="status-readout"><span class="status-pulse"></span><span id="sector-status">PREPARING SECTORS</span></div><div class="status-separator"></div><div id="coordinate-status">X +0.0 / Z +0.8 KM</div><div class="status-controls">DRAG TO PAN <span>·</span> SCROLL TO ZOOM <span>·</span> RIGHT-DRAG TO ORBIT <span>·</span> WASD TO MOVE</div></footer>

    <div id="atlas-overlay" class="atlas-overlay" role="dialog" aria-modal="true" aria-labelledby="atlas-heading" aria-hidden="true">
      <header class="atlas-topbar"><div><div class="atlas-kicker">MORROW REACH <span>/</span> REGIONAL SURVEY 01</div><h2 id="atlas-heading">The regional atlas<span>.</span></h2></div><button id="close-atlas" class="atlas-close" aria-label="Close atlas">${icon.close}<span>CLOSE MAP</span><kbd>ESC</kbd></button></header>
      ...
      <aside class="atlas-inspector"><div class="inspector-index">SURVEY INDEX / 001—012</div><h3>Land first.<br><em>Everything follows.</em></h3><p>The footprint is reserved. Roads connect the regions; the districts themselves remain unbuilt.</p><div class="atlas-hover" id="atlas-hover">HOVER A FOOTPRINT TO INSPECT</div><div class="inspector-divider"></div><div class="inspector-selected" id="inspector-selected"></div><button class="inspect-action" id="atlas-explore">EXPLORE THIS AREA ${icon.arrow}</button><div class="inspector-legend"><div><i class="legend-box legend-highway"></i> HIGHWAY / PARKWAY</div><div><i class="legend-box legend-secondary"></i> LOCAL / RURAL ROUTE</div><div><i class="legend-box legend-river"></i> FRESH + TIDAL WATER</div><div><i class="legend-box legend-reserve"></i> FUTURE DISTRICT LIMIT</div></div></aside></div>
    </div>
  </main>
`;
...
DISTRICTS.forEach((district,i)=>{
  ...
  button.innerHTML=`<span class="district-no">${String(i+1).padStart(2,'0')}</span><span class="district-color" style="--district-color:${district.color}"></span><span class="district-name">${district.name}<small>${district.subtitle}</small></span><span class="district-arrow">↗</span>`;
...
function districtDetails(district:District,full=false){
  ...
  if(full)return `<div class="inspector-district-type">${groupLabel(district)} <span>· RESERVED</span></div><h4>${district.name}</h4><div class="inspector-subtitle">${district.subtitle}</div><dl><div><dt>LAND RESERVED</dt><dd>${area} km²</dd></div><div><dt>FOCUS ELEVATION</dt><dd>${elevation} m</dd></div><div><dt>ROAD ACCESS</dt><dd class="dd-access">${district.access}</dd></div></dl>`;
  return `<div class="selected-card-top"><span><i></i> SELECTED LOCATION</span><span>${String(DISTRICTS.indexOf(district)+1).padStart(2,'0')} / 12</span></div><div class="selected-card-name"><div><small>${groupLabel(district)}</small><h2>${district.name}</h2><p>${district.subtitle}</p></div><div class="card-height"><strong>${elevation}<small>M</small></strong><span>ELEV.</span></div></div><div class="selected-card-bottom"><div><span>RESERVED AREA</span><strong>${area} KM²</strong></div><div><span>ROAD ACCESS</span><strong>${district.access}</strong></div></div><button class="card-focus" id="focus-selected">FOCUS LOCATION ${icon.arrow}</button>`;
}
...
selectDistrict(selectedId,false);
